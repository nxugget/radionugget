...
# Introduction
What we call **GPS** (Global Positioning System) is just one type of **GNSS** operated by the **United States** since the 1970s. There are others, like [Galileo](https://fr.wikipedia.org/wiki/Galileo_(syst%C3%A8me_de_positionnement)) in **Europe**, [GLONASS](https://fr.wikipedia.org/wiki/GLONASS) in **Russia**, or [BeiDou](https://fr.wikipedia.org/wiki/Beidou) in **China**.  
...
![Schema Référentiel géocentrique](/images/blog/2024/gps/gps1.svg)
...
# Triangulation
All the satellites’ clocks are synchronized to a source known as [GPS Time](https://fr.wikipedia.org/wiki/Synchronisation_GPS). When your **GNSS** receiver (for example, your smartphone) picks up signals from at least **4 satellites**, it can calculate your position by measuring **4 pseudo-ranges**.
...
![Schema décalage d'horloge](/images/blog/2024/gps/gps3.svg)
To correct these issues, each satellite transmits a unique random code. Your receiver generates an identical code in sync with the satellite’s transmission and compares the time shift between sending and receiving. This time offset is then used in the formula:  
`P = c * Δt`  
...
We won’t dive into the nitty-gritty of solving these equations—it gets pretty involved—but there are two primary methods: ([Least Squares Method](https://fr.wikipedia.org/wiki/M%C3%A9thode_des_moindres_carr%C3%A9s)) and ([Kalman Filter](https://fr.wikipedia.org/wiki/Filtre_de_Kalman)). If you’re curious, check them out!  
...
Keep in mind that latitude and longitude are based on an **ellipsoidal approximation** of **Earth**. It makes sense: you’re converting 3D data into a 2D map. The best way to represent our blue planet (which is roughly spherical) is by using a **reference ellipsoid** defined by the [WGS 84](https://en.wikipedia.org/wiki/World_Geodetic_System#WGS_84) model.
